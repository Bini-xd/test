...
\033[38;2;255;127;0;1m ╔══════════════════════════════════════════╗
 AUTHOR   {O}• Karma David
 GITHUB   {O}• https://github.com/Karma-Kh3n
 FB       {O}• https://me.fb/Karma428
 ╚══════════════════════════════════════════╝{C}
""")
try:
    # python 2
...
class instagram:
    ...
    def logo(self):
        for i in external:
            ...
            banner()
            print(f"""{O}
\033[38;2;255;127;0;1m[{O}•{C}]WELCOME          {O}•{nama}
[{O}•{C}]USERNAME         {O}•{self.username}
[{O}•{C}]FOLLOWERS        {O}•{followers}
[{O}•{C}]FOLLLOWING       {O}•{following} 
...
[03] CRACK FROM FOLLOWING {O}•[✓]
[04] CHECK CRACK RESULT 
{C}[05] VIEW  CRACK RESULT 
[•E] EXIT
	""")
...
#					"enc_password": "#PWD_INSTAGRAM_BROWSER:0:{}:{}".format(random.randint(1000000000, 99999999999),pw),
    ...
    def menu(self):
        ...
        if c=='':
            ...
            mas=input('CRACK FROM MULTIPLE ID? y/t >  ')
            if mas in ['y','Y']:
                masal(self)
            elif mas in ['t','T']:
                massal(self)
